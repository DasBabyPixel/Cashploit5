package de.cashploit5.v1_8_8.tornado;
...
public class Tornado {
	...
	private void setForBlock(Location loc, int hight) {
		...
		if (mat != null && mat != Material.AIR) {
			...
			FallingBlock block = loc.getWorld().spawnFallingBlock(loc.add(0, 3, 0), mat, (byte)0);
		...
		throwingID = Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.getPlugin(), new Runnable() {
			...
			public void run() {
				...
				for (FallingBlock b : blocks) {
					...
					if(new Random().nextInt(Math.abs(dist)) == 0 || h.distance(l) > d) {
						l.setY(oldY);
						double ax = h.getX();
						double az = h.getZ();
						double bx = l.getX();
